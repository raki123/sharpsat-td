...
public:
	...
	T_num solve(const sspp::Instance<T_num>& pp_ins, const sspp::TreeDecomposition& tdec);
	...
	if (Weighted()) {
		if (stack_.size() >= 2 && Instance<T_num>::lit_weights_.size() > 0) {
			if (Instance<T_num>::dec_cands_[stack_.size()][lit.var()] == Instance<T_num>::dec_cands_[stack_.size()][0]) {
				...
				stack_.top().dec_weights.insert(make_pair(lit, Instance<T_num>::lit_weights_[lit]));
			}
		}
	}
...
inline void Solver<T_num>::unsetLiteral(LiteralID lit) {
	if (Weighted()) {
		if (stack_.size() >= 2 && Instance<T_num>::lit_mul_[lit.var()]) {
			stack_.top().dec_weights.erase(lit);
		}
...
template <class T_num>
T_num Solver<T_num>::solve(const sspp::Instance<T_num>& pp_ins, const sspp::TreeDecomposition& tdec) {
	stopwatch_.start();
